import aiosqlite
import datetime
from api import get_orders_info
    ...
    async with aiosqlite.connect('database.db', check_same_thread=False) as db:
        try:
            ...
            await db.execute('CREATE TABLE orders(id INTEGER PRIMARY KEY, user_id INT, order_id INT,'
                             'name TEXT, cost INT, quantity INT, status INT, service_id)')
            await db.execute('CREATE TABLE promo(id INTEGER PRIMARY KEY, service_id INT, name TEXT,'
                             'activations INT, min_order INT)')
            await db.execute('CREATE TABLE checks(id INTEGER PRIMARY KEY, user_id INT, service_id INT,'
                             'min_order INT, quantity INT)')
            await db.commit()
...
async def get_orders(user_id: int = 0):
    async with aiosqlite.connect('database.db') as db:
        cur = await db.cursor()
        try:
            if user_id:
...
async def add_order(user_id, order_id, cost, quantity, status, service_id):
    async with aiosqlite.connect('database.db') as db:
        await db.execute('INSERT INTO orders (user_id, order_id, cost, quantity, status, service_id, datetime)'
                         'VALUES(?, ?, ?, ?, ?, ?, datetime("now", "+3 hours"))', (user_id, order_id, cost, quantity, status, service_id))
...
async def update_orders(orders):
    async with aiosqlite.connect('database.db') as db:
        for order in orders:
            await db.execute('UPDATE orders SET status = ?', (orders[order]['status'],))
        await db.commit()
